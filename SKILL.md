...
description: 微服务和跨团队系统 AI 友好化 / AI 协作推进 skill。当用户需要为单个代码仓库生成 AI 友好化文档时使用 repo-mode，包括生成 AGENTS.md、README.md、架构、领域模型、API、决策日志和运维手册；当用户需要推进跨微服务、跨小组需求时使用 system-mode，包括生成项目级跨服务协作 skill、AI 上下文地图、全局系统上下文、服务目录、服务依赖图、跨服务契约索引、功能路由表、跨服务 RFC、小组任务单和可复制给各小组 AI 的 vibe coding prompt。触发场景："帮我做 AI 友好化"、"生成 AGENTS.md"、"跨服务 AI 协作"、"多微服务需求拆分"、"生成跨团队 RFC"、"生成小组任务单"、"让 AI 理解多个微服务关系"、"ai-friendly"。
---
...
2. **system-mode：跨服务 AI 协作推进** —— 为多微服务、多小组场景生成“项目级消费 skill + 全局事实上下文”。项目级 skill 负责指导 AI 如何加载上下文、分析影响面、生成 RFC 和拆分任务；`ai-system-context/` 负责保存服务目录、服务关系图、契约索引、功能路由、服务卡片等事实资料。目标是统一上下文和任务拆分，不是让一个 AI 越权统一修改所有服务代码。
...
参考 `references/AGENTS.md`，按以下要求填充：
...
参考 `references/README.md`。必须包含：服务职责、技术栈、快速启动、目录结构、文档索引、维护团队。
...
参考 `references/api-templates.md`。读取 handler / controller / router 层，以及 proto / openapi / MQ schema 文件；每个接口输出：业务描述、Method+路径 / RPC 方法 / Topic、请求/响应参数表格、错误码含义、调用方、兼容性要求和注意事项。
...
**输出路径：** `./docs/dev-guide.md`

参考 `references/dev-guide-templates.md`。基于仓库实际代码结构，按「场景 → 定位 → 步骤 → 检查」模式生成新人可用的修改指导手册：

1. 从目录结构和已有代码推断常见修改场景（新接口、改逻辑、加字段、加 MQ、改配置、排查问题）
2. 每个场景定位到具体文件路径（非通用路径，用实际目录名）
3. 操作步骤基于实际代码结构，不编造不存在的目录
4. 常见陷阱从代码注释 `// NOTE:` `// IMPORTANT:` `// FIXME:` 和业务规则中提取
5. 末尾生成「项目修改速查表」，按实际目录组织修改规则
6. 无法推断的陷阱或规则标注 `[待补充]`


## system-mode：跨服务 AI 协作任务

### 任务 H0：生成项目级跨服务协作 skill

**输出路径：** `.codebuddy/skills/cross-service-ai-collaboration/SKILL.md`

参考 `references/cross-service-project-skill-template.md`。该 skill 是 system-mode 的核心产物，负责定义 AI 的触发场景、上下文加载策略、跨服务影响面分析流程、RFC 生成流程、小组任务单拆分流程、契约变更默认规则和禁止行为。该 skill 不重复保存业务事实，只引用 `ai-system-context/` 下的事实资料。

### 任务 H1：生成 AI 上下文地图

**输出路径：** `./ai-system-context/CONTEXT-MAP.md`

参考 `references/cross-service-ai-collaboration-templates.md`。`CONTEXT-MAP.md` 用结构化表格说明每个上下文文件的职责、读取时机、依赖关系和典型输出，供项目级 skill 按任务类型选择最小必要上下文。
...
Step 9: 生成 docs/dev-guide.md
Step 10: 文档索引一致性校验 —— 对照 README 的"文档索引"，逐条核验 docs/ 下文件是否齐全，缺失则补生成骨架
Step 11: 输出完成摘要 + 所有 [待确认] 项列表 + 本次补齐的骨架文档清单
Step 12: 进入待确认项交互确认流程（Phase 1→2→3），逐项与用户正反面澄清，更新文档
```
...
Step 2: 生成 .codebuddy/skills/cross-service-ai-collaboration/SKILL.md，作为消费 ai-system-context 的项目级 skill
Step 3: 生成 ai-system-context/CONTEXT-MAP.md，作为上下文地图
Step 4: 生成 ai-system-context/SYSTEM.md
Step 5: 生成 SERVICE-CATALOG.md 和 services/<service-name>.md
Step 6: 生成 SERVICE-GRAPH.md
Step 7: 生成 CONTRACTS.md
Step 8: 生成 FEATURE-ROUTING.md
Step 9: 如输入了具体需求，生成 rfcs/<feature-name>.md
Step 10: 如生成了 RFC，继续生成 tasks/<feature-name>/<team-or-service>.md
Step 11: 生成或更新 VIBE-CODING-PROMPTS.md 和 CHANGE-PLAYBOOKS.md
Step 12: 输出推进摘要 + 各小组待确认项 + 契约风险 + 建议推进顺序
Step 13: 进入待确认项交互确认流程（Phase 1→2→3），逐项与用户正反面澄清，更新文档
```
...
AI 友好化生成的文档中会包含 `[待确认]`、`[需人工确认]`、`[待补充]` 占位标记。**所有文档生成完成后，必须进入待确认项交互确认流程，逐一与用户澄清，直至全部确认完毕。**
...
| 2 | `docs/architecture.md` | 核心流程 | 订单超时取消机制 | `[待补充]` | ⏳ |
| ... | | | | | |
```

**规则：**
- 确认项内容用业务语言概括，不直接复制原文；让用户能快速理解需要确认什么
- 按来源文档分组排序，同一文档内的项连续展示
- 状态列初始为 ⏳，确认后改为 ✅

### Phase 2：逐项确认

按清单顺序，**逐项**与用户交互确认。每项确认必须包含**正反面澄清**：
...
1. **正面澄清**：直接询问该项的正确/最可能值是什么，给出 2-3 个常见选项引导
2. **反面澄清**：提出与正面相反或另一种可能的场景，确认是否排除
3. 若用户回答正面，用确认值替换 `[待确认]` / `[待补充]` 占位符
4. 若用户回答反面，追问具体细节，再替换
5. 若用户暂时无法确认，保留标记并在清单中标注 🔶（待后续确认），不跳过不删除
6. **每确认一项，立即更新对应文档文件**，将占位符替换为确认值

### Phase 3：完成校验

所有项确认完毕后（含 🔶 延后确认项），输出确认摘要：
...
| # | 来源文档 | 确认项 | 原占位值 | 确认值 |
|---|---------|-------|---------|-------|
| 1 | AGENTS.md | 支付服务调用协议 | [待确认] | HTTP 同步调用，POST /api/pay |

### 延后确认项
| # | 来源文档 | 确认项 | 原因 |
|---|---------|-------|------|
| 2 | docs/architecture.md | 订单超时取消机制 | 需要与业务方确认规则 |

⚠️ 延后确认项仍保留 [待确认] / [待补充] 标记，请在信息就绪后手动补充或重新触发确认流程。
```

**强制规则：**
- 不允许跳过任何待确认项；每项至少经过正反面澄清
- 不允许编造确认值；用户未明确回答时，保留占位标记
- 确认值写入文档后，原 `[待确认]` / `[需人工确认]` / `[待补充]` 标记必须移除，替换为确认值
- 如果确认过程中发现新的待确认内容，追加到清单末尾
...
- [ ] 没有编造不存在的接口、字段或业务规则
- [ ] 文档间引用路径正确
- [ ] 待确认项交互确认流程已完成：所有 `[待确认]`/`[需人工确认]`/`[待补充]` 项已逐项正反面澄清，确认值已写入文档

### repo-mode 检查
- [ ] "添加新功能步骤"基于实际目录结构
- [ ] 枚举字段每个值都有业务含义说明
- [ ] README 文档索引中的每个条目，在 `docs/` 下都有对应文件（至少是骨架）
- [ ] AGENTS.md 包含「新人快速入门」章节，含修改导航表和提交前检查清单
- [ ] docs/dev-guide.md 基于实际目录结构生成，场景定位到具体文件路径（非通用路径）

### system-mode 检查
- [ ] 已生成 `.codebuddy/skills/cross-service-ai-collaboration/SKILL.md`，并明确触发场景、上下文加载策略、输出格式和禁止越权规则
- [ ] 项目级 skill 不重复保存业务事实，只引用 `ai-system-context/` 下的事实资料
- [ ] 已生成 `ai-system-context/CONTEXT-MAP.md`，并用结构化表格说明每个文件的职责、读取时机、依赖关系和典型输出
- [ ] 核心上下文文档开头包含 `context_type`、`purpose`、`read_when`、`depends_on`、`update_when` 等 AI 元信息
- [ ] 服务职责边界清晰区分“负责 / 不负责”
...
### repo-mode 模板
- `references/AGENTS.md` — AGENTS.md 完整模板（含新人快速入门）
- `references/README.md` — README.md 完整模板
...
- `references/api-templates.md` — `docs/api.md` API 文档模板
- `references/decision-log-templates.md` — `docs/decision-log.md` 设计决策日志模板
- `references/runbook-templates.md` — `docs/runbook.md` 运维手册模板
- `references/dev-guide-templates.md` — `docs/dev-guide.md` 代码修改快速指导手册模板

### system-mode 模板
- `references/cross-service-project-skill-template.md` — `.codebuddy/skills/cross-service-ai-collaboration/SKILL.md` 项目级消费 skill 模板，定义 AI 工作流和上下文加载策略
- `references/cross-service-ai-collaboration-templates.md` — 跨服务上下文地图、全局上下文、服务目录、服务卡片、服务依赖图、契约索引、功能路由表、跨服务 RFC、小组任务单、变更 SOP 和 vibe coding prompt 模板
